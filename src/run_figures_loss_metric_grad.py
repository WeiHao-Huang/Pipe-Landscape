...
def plot_1D_landscape(step1, step2, lr):
    ...
    for i, alpha in enumerate(interp_factors):
        ...
        metric = compute_loss_change(interp_model, eval_batches, lr).item()
    
        perplexities.append(metric)
        #print(f"  Completed interpolation point {i+1}/{n_points}, loss = {perplexity:.2f}")
        print(f"Processing interpolation point {i+1}/{n_points}, alpha = {alpha:.2f} grad_norm: {metric}")
    ...
    plt.savefig(f'/chenyupeng/old_files/yupeng_gpt/WSD/jaggi-lr/src/save_file_grad_norm/1d_landscape_{step1}_{step2}_grad_norm.png', dpi=600,bbox_inches='tight')
    plt.show()
...
def plot_2D_landscape(step1,step2,step3, lr):
    ...
    for i, alpha in enumerate(interp_factors_x):
        for j, beta in enumerate(interp_factors_y):
            ...
            print(f"Processing interpolation point ({i+1}/{n_points}, {j+1}/{n_points}), alpha = {alpha:.2f}, beta = {beta:.2f} grad_norm: {metric}")
    ...
    plt.savefig(f'/chenyupeng/old_files/yupeng_gpt/WSD/jaggi-lr/src/save_file_grad_norm/2d_landscape_{step1}_{step2}_{step3}_grad_norm.png', dpi=600,bbox_inches='tight')
...
#
#
for i in range(6000, 21000, 1000):
