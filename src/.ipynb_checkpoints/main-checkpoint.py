...
from optim.utils import cos_inf_schedule, wsd_schedule, wsd_schedule_rewarm

#print("Loading base.py from:", optim.base.__file__)
def main(args):
...
#        torch.cuda.set_device(local_rank)
    ...
    if distributed_backend.is_master_process() and args.wandb:
        wandb.init(
            ...
            name=exp_name,
            config=vars(args)
            # dir="/mntcephfs/lab_data/chenyupeng/senmiao_jaggi_exp_results/wandb"
        )
    ...
    if args.scheduler != "none":
        ...
        if args.scheduler in ["cos", "linear"]:
            ...
            if args.resume_from and args.resume_from2 and args.resume_from3 and args.resume_warmup>0:
                print(f"rewarm up at {args.resume_warmup}")
                lambda_schedule = wsd_schedule_rewarm(
                    rewarm_up_step = args.resume_warmup,
                    n_iterations=args.iterations,
                    n_warmup=args.warmup_steps,
                    ...
                    wsd_decay_steps=args.wsd_decay_steps if hasattr(args, 'wsd_decay_steps') else None,
                )
            else:    
