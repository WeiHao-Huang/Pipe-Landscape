...
from models.base_senmiao import CausalSelfAttention, GPTBase
from models.base_senmiao import preconditioning, PCNorm, LayerChoice
...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        # 在 MLP 模块中使用 'mlp' 作为模块名
        self.w1 = LayerChoice(config, 'mlp', 'w1', 
                            config.n_embd, hidden_dim, 
                            bias=False)
        self.w2 = LayerChoice(config, 'mlp', 'w2', 
                            config.n_embd, hidden_dim, 
                            bias=False)
    ...
    def forward(self, x):
        # print(f"w1 shape: {self.w1.layer.weight.shape}")
        # print(f"w2 shape: {self.w2.layer.weight.shape}")
        # print(f"c_proj shape: {self.c_proj.layer.weight.shape}")
...
class LlamaAttention(nn.Module):
    ...
    def forward(self, x, freqs_cis):
        ...
        # reshape heads (B, T, n_head, head_dim) → (B, n_head, T, head_dim)
...
class Llama(GPTBase):
    def __init__(self, config):
        ...
        #     if pn.endswith("c_proj.weight") or pn.endswith("c_proj.layer.weight"):
