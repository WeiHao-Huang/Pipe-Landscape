...
def compute_loss_change(model, eval_batches, lr):
    ...
    return original_loss-after_loss
...
def plot_1D_landscape(step1, step2, lr):
    ...
    for i, alpha in enumerate(interp_factors):
        ...
        print(f"Processing interpolation point {i+1}/{n_points}, alpha = {alpha:.2f} loss changes: {metric}")
    ...
    plt.savefig(f'/chenyupeng/old_files/yupeng_gpt/WSD/jaggi-lr/src/save_file_loss_change/1d_landscape_{step1}_{step2}_lr{lr}.png', dpi=600)
    plt.show()
...
def plot_2D_landscape(step1,step2,step3, lr):
    ...
    for i, alpha in enumerate(interp_factors_x):
        for j, beta in enumerate(interp_factors_y):
            ...
            print(f"Processing interpolation point ({i+1}/{n_points}, {j+1}/{n_points}), alpha = {alpha:.2f}, beta = {beta:.2f} loss changes: {metric}")
    ...
    plt.savefig(f'/chenyupeng/old_files/yupeng_gpt/WSD/jaggi-lr/src/save_file_loss_change/2d_landscape_{step1}_{step2}_{step3}_loss_changes_lr{lr}.png', dpi=600)
